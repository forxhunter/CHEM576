...
```
> [!NOTE] Can you estimate the time complexity for this alogrithm implementation? 
...
The overall architecture of AlphaFold 3 (AF3) is similar to AlphaFold 2 (AF2). Both systems use a **large network trunk** (called the Evoformer in AF2 and the Pairformer in AF3) to refine a pairwise representation of the molecular complex. This representation is then passed to either the StructureModule (in AF2) or the DiffusionModule (in AF3) to generate explicit atomic positions.
>[!NOTE]Large network trunk: the core part of the neural network architecture responsible for processing and refining the input data to generate meaningful representations of the molecular structure. It acts as the backbone of the model, handling the bulk of the computational work. It processes input data (e.g., sequences, multiple sequence alignments, or pairwise interactions) and refines it into a pairwise representation. For AF2, its evoformer; AF3, it is pairformer.
...
>[!WARNING] AF3 only support double strand DNA input. IF you want to try single strand, use RoseTTAFold2NA, https://github.com/uw-ipd/RoseTTAFold2NA.
...
> [!NOTE] if you want to compare with evoformer achitecture, please check: Original AF2 paper: Jumper, J., Evans, R., Pritzel, A. et al. Highly accurate protein structure prediction with AlphaFold. Nature 596, 583–589 (2021). https://doi.org/10.1038/s41586-021-03819-2
...
>[!NOTE]LDDT: The LDDT score (Local Distance Difference Test) is a metric used to evaluate the accuracy of predicted protein structures. It ranges from 0 to 100, where higher socres indicate better local accuracy.
